...
module.exports = {
  'apply with namespace' : function ( test ) {
    test.expect( 3 );
    var fn = lineCutter( 'namespace1', mod );
    fn( req, res, next );
    test.ok( req.prop1 === 'a', 'Properties should not be overwritten' );
    test.ok( req.namespace1, 'Namespace should be created' );
    test.ok( req.namespace1.name === 'line-cutter', 'New properties should be under namespace' );
    test.done();
  },
  'apply with namespace, force' : function ( test ) {
    test.expect( 2 );
    var fn = lineCutter( 'namespace1', { node: 'superrocks', infinite: 'recursion' });
    fn( req, res, next );
  ...
  'apply with namespace, do not force' : function ( test ) {
    test.expect( 4 );
    var fn = lineCutter( 'namespace1', { name: 'newName', node: 'newNode', peanut: 'butter' }, false);
    fn( req, res, next );
    test.ok( req.prop1 === 'a', 'Properties should not be overwritten' );
    test.ok( req.namespace1, 'Namespace should be created' );
    test.ok( req.namespace1.name === 'line-cutter', 'Prop should not be overwritten, force == false');
  ...
  'apply without namespace' : function ( test ) {
    test.expect( 3 );
    var fn = lineCutter({ prop4: 'd', prop2: '!' });
    fn( req, res, next );
  ...
  'apply without namespace, do not force' : function ( test ) {
    test.expect( 3 );
    var fn = lineCutter({ prop1: '@', prop3: '@', prop5: '@' }, false);
    fn( req, res, next );
